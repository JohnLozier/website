...
const Item = (props: { shown: Accessor<Boolean>, name: string, link: string, delay: number }) => {
	...
	return <Link class={ `duration-1000 delay-${ props.delay } group pointer cursor-none p-2 transition-[transform,margin] block w-fit${ props.shown() ? "" : " -translate-x-full -ml-4" }` } href={ props.link }>
